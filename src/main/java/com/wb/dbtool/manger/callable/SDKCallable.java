...
public class SDKCallable implements Callable {
    ...
    public Boolean call() throws Exception {
...

        {
            ...
            Template template = velocityEngine.getTemplate("/templates/Java_sdk/module/controller/ApiController.vm", "UTF-8");
            outputVM(file, template, velocityContext);
        }
        ...
        velocityContext.put("domain", domain);
        {
            ...
            Template template = velocityEngine.getTemplate("/templates/Java_sdk/ApiEntity.vm", "UTF-8");
            outputVM(file, template, velocityContext);
        }
        {
            File file = new File(frameWork.getAbsolutePath() + File.separator + "ApiClient.java");
            Template template = velocityEngine.getTemplate("/templates/Java_sdk/ApiClient.vm", "UTF-8");
            outputVM(file, template, velocityContext);
        }
        {
            File file = new File(frameWork.getAbsolutePath() + File.separator + "ApiRequest.java");
            Template template = velocityEngine.getTemplate("/templates/Java_sdk/ApiRequest.vm", "UTF-8");
            outputVM(file, template, velocityContext);
        }
        {
        ...
        }
        {
            ...
            Template template = velocityEngine.getTemplate("/templates/Java_sdk/Base64Util.vm", "UTF-8");
            outputVM(file, template, velocityContext);
            revert(file);
        }
//        {
...
//        }
        ...
        }
        {
            ...
            Template template = velocityEngine.getTemplate("/templates/Java_sdk/FileUploadRequest.vm", "UTF-8");
            outputVM(file, template, velocityContext);
        }
        {
            File file = new File(frameWork.getAbsolutePath() + File.separator + "FileUploadResponse.java");
            Template template = velocityEngine.getTemplate("/templates/Java_sdk/FileUploadResponse.vm", "UTF-8");
            outputVM(file, template, velocityContext);
        }
        {
            File file = new File(frameWork.getAbsolutePath() + File.separator + "JsonParser.java");
            Template template = velocityEngine.getTemplate("/templates/Java_sdk/JsonParser.vm", "UTF-8");
            outputVM(file, template, velocityContext);
        }
//        {
//            File file = new File(frameWork.getAbsolutePath() + File.separator + "Main.java");
//            Template template = velocityEngine.getTemplate("/templates/Java_sdk/Main.vm", "UTF-8");
//            outputVM(file, template, velocityContext);
//        }
        {
            File file = new File(frameWork.getAbsolutePath() + File.separator + "MD5Util.java");
            Template template = velocityEngine.getTemplate("/templates/Java_sdk/MD5Util.vm", "UTF-8");
            outputVM(file, template, velocityContext);
        }
        {
            File file = new File(frameWork.getAbsolutePath() + File.separator + "RSAUtil.java");
            Template template = velocityEngine.getTemplate("/templates/Java_sdk/RSAUtil.vm", "UTF-8");
            outputVM(file, template, velocityContext);
        }
        {
